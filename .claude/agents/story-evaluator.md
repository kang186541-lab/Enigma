...
**신규 퀴즈 3종 평가 (voice_power / debate_battle / npc_rescue):**

- **voice_power**: Stone 반응 레벨이 챕터별로 차별화되는가?
  - Ch1: 약한 빛 → Ch3: 중간 → Ch5: 눈부신 빛
  - 발음 점수(90%+)와 Stone 이펙트가 연결되는가?
  - 발음=파워 시스템이 스토리와 자연스럽게 통합되는가?

- **debate_battle**: 라운드 수가 레벨에 적합한가?
  - Ch4: 3라운드 (적정), Ch5: 5라운드 (최종 보스급)
  - 라운드별 난이도가 점진적으로 상승하는가?
  - 필수 표현(minExpressions)을 사용해야 통과하는 구조인가?

- **npc_rescue**: 점진적 도입(progressive intro)인가?
  - Ch2: 2단계 구출 → Ch4: 3단계 구출
  - 각 단계가 TPRS 스테이지와 매핑되는가?
  - 구출 성공 시 NPC 관계도 상승이 연결되는가?
