...
- Indonesian-native course metadata pass: all 5 `UNITS[].title` objects and all 30 `days[].topic` objects in `lib/dailyCourseData.ts` now include `id`, so Home, Rudy course rows, Rudy lesson headers, daily lesson bridge, and chat-room lesson context no longer fall back to English for those titles. A sidecar audit found 120 active MissionTalk `situation` objects still missing `id`; those are intentionally left for a separate content-only translation pass because they are user-facing scenario sentences, not short metadata labels.
