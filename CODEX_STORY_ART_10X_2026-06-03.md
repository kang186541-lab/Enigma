...
- Dialogue staging pass: story dialogue backdrops now render a subtle lower-scene depth gradient and angled ground plane behind the character layer. This makes the cutout sprites read more like characters standing in the scene, closer to visual-novel staging, without changing story text, quiz data, progression, provider routing, language scope, or Japanese scope.
- Dialogue-stage regression guard: `scripts/verify-story-stage.ts` now locks the active visual-novel staging contract. It verifies all 9 dialogue backdrops and 16+ dialogue sprites are wired to existing files, current/next-two scene prefetch is present, backdrop shade/glow/depth/ground layers are rendered, character backlight/contact shadow/occlusion/speaker motion remain applied, and the removed legacy motion-comic preview path stays disconnected. `npm run verify:quality` now runs this guard.
