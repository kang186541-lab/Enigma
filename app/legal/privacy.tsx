...
const T: Record<string, Record<LangKey, string>> = {
  title:        { ko: "개인정보처리방침",                       en: "Privacy Policy",                       es: "Política de Privacidad",              id: "Kebijakan Privasi" },
  ...
  intro_h:      { ko: "1. 머리말",                              en: "1. Introduction",                      es: "1. Introducción",                     id: "1. Pendahuluan" },
  intro_b: {
    ko: "LinguaAI(이하 \"서비스\")는 한국어, 영어, 스페인어, 인도네시아어 학습을 돕는 학습용 모바일·웹 애플리케이션입니다. 본 방침은 「개인정보 보호법」, EU 일반개인정보보호법(GDPR) 및 관련 법령에 따라 회사가 어떤 정보를 수집하고, 어떻게 사용하며, 어떻게 보관·파기하는지 설명합니다. 본 방침을 정독한 뒤 서비스를 이용해 주세요.",
    en: "LinguaAI (the \"Service\") is a mobile and web learning application that helps users study Korean, English, Spanish, and Indonesian. This Privacy Policy explains, in accordance with the Korean Personal Information Protection Act (PIPA), the EU General Data Protection Regulation (GDPR), and other applicable laws, what data we collect, how we use it, and how we store and delete it. Please read it carefully before using the Service.",
    es: "LinguaAI (el \"Servicio\") es una aplicación móvil y web que ayuda a estudiar coreano, inglés, español e indonesio. Esta Política explica, conforme a la Ley Coreana de Protección de Información Personal (PIPA), al Reglamento General de Protección de Datos de la UE (RGPD) y a otras normas aplicables, qué datos recopilamos, cómo los usamos y cómo los conservamos y eliminamos.",
    id: "LinguaAI (\"Layanan\") adalah aplikasi pembelajaran seluler dan web yang membantu pengguna belajar bahasa Korea, Inggris, Spanyol, dan Indonesia. Kebijakan Privasi ini menjelaskan, sesuai dengan Undang-Undang Perlindungan Informasi Pribadi Korea (PIPA), Peraturan Perlindungan Data Umum Uni Eropa (GDPR), dan peraturan lain yang berlaku, data apa yang kami kumpulkan, bagaimana kami menggunakannya, serta bagaimana kami menyimpan dan menghapusnya. Mohon baca dengan saksama sebelum menggunakan Layanan.",
  },

  controller_h: { ko: "2. 개인정보 처리자(컨트롤러)",           en: "2. Data Controller",                   es: "2. Responsable del Tratamiento",       id: "2. Pengendali Data" },
  controller_b: {
    ...
    id: "Pengendali data untuk Layanan ini adalah tim operasional LinguaAI. Untuk pertanyaan terkait privasi atau untuk menggunakan hak-hakmu, hubungi privacy@linguaai.example. Penduduk Korea dapat menghubungi Petugas Perlindungan Informasi Pribadi kami di alamat tersebut. Pengguna di UE/EEA dapat menghubungi alamat yang sama untuk hal-hal terkait Pasal 27 GDPR.",
  },

  collect_h:    { ko: "3. 수집하는 개인정보 항목",              en: "3. What We Collect",                   es: "3. Qué Datos Recopilamos",             id: "3. Data yang Kami Kumpulkan" },
  collect_b: {
    ko: "회사는 서비스 제공에 필요한 최소한의 정보만을 수집합니다.\n\n(a) 계정 정보 — 이메일 주소(Google OAuth 또는 매직 링크 로그인을 통해), Supabase Authentication이 발급하는 사용자 ID, 그리고 Google 로그인을 사용하실 경우 Google이 제공하는 프로필 정보(이름, 프로필 사진 URL).\n(b) 학습 진도 — 경험치(XP), 레벨, 연속 학습일(streak), 학습한 단어 수, 마지막 학습일, 모국어·학습언어 설정. 이 데이터는 Supabase의 linguaai_user_progress 테이블에 저장됩니다.\n(c) 음성 데이터 — 발음 연습 시 사용자가 녹음한 음성은 Microsoft Azure Speech 서비스로 전송되어 발음 정확도를 평가합니다. 회사는 이 음성을 별도로 저장하지 않으며, Azure도 마이크로소프트의 \"No-Retention\" 정책에 따라 즉시 폐기합니다.\n(d) 대화/작문/이미지 텍스트 — 튜터 채팅, 작문 연습, 손글씨·이미지 인식 등에서 사용자가 입력한 콘텐츠는 설정된 제3자 AI 제공자(예: DeepSeek, OpenAI, Anthropic Claude, Google Gemini)로 전송되어 응답·교정·인식을 생성하는 데 사용됩니다. 회사는 원문 프롬프트와 음성을 서버에 저장하지 않으며, 각 제공자의 데이터 처리 조건 및 현재 프로젝트 설정에 따라 처리됩니다.\n(e) 기기 및 로그 정보 — 앱 충돌 로그, 운영체제 종류, 앱 버전. 보안·품질 개선 목적으로만 사용합니다.\n(f) 알림 토큰 — 매일 학습 알림을 켠 경우 Expo Push 토큰을 저장합니다.\n\n회사는 14세 미만 아동의 개인정보를 의도적으로 수집하지 않습니다.",
    en: "We collect only the minimum information needed to operate the Service.\n\n(a) Account information — your email address (via Google OAuth or magic-link), the Supabase user ID assigned at sign-up, and, if you use Google sign-in, the profile information Google returns (display name, avatar URL).\n(b) Learning progress — XP, level, daily streak, number of words learned, last session date, native and target language settings. These are stored in our Supabase table linguaai_user_progress.\n(c) Voice recordings — When you record yourself for pronunciation practice, the audio is sent to Microsoft Azure Speech for scoring. We do not retain the recording on our servers, and Azure follows a \"no-retention\" policy and discards the audio after scoring.\n(d) Conversation / writing / image text — Content you type or submit for tutor chat, writing practice, handwriting, or image recognition may be sent to configured third-party AI providers (for example DeepSeek, OpenAI, Anthropic Claude, or Google Gemini) to produce replies, corrections, or recognition results. We do not store raw prompts or voice audio on our servers; provider handling is governed by each provider's data-processing terms and the current project settings.\n(e) Device and log data — crash logs, OS type, app version. Used only for security and product quality.\n(f) Push token — if you turn on the daily reminder, we store an Expo push token tied to your device.\n\nWe do not knowingly collect data from children under 14 (Korea) or 16 (EEA).",
    es: "Recopilamos solo la información mínima necesaria para operar el Servicio.\n\n(a) Información de cuenta — tu dirección de correo electrónico (vía Google OAuth o enlace mágico), el ID de usuario asignado por Supabase y, si usas Google, la información de perfil devuelta por Google (nombre, URL del avatar).\n(b) Progreso de aprendizaje — XP, nivel, racha diaria, número de palabras aprendidas, fecha de la última sesión, idioma nativo y de estudio. Se guarda en nuestra tabla linguaai_user_progress de Supabase.\n(c) Grabaciones de voz — Cuando te grabas para practicar pronunciación, el audio se envía a Microsoft Azure Speech para evaluarlo. No conservamos la grabación; Azure sigue una política de \"no retención\" y descarta el audio tras la evaluación.\n(d) Texto de conversación / escritura / imagen — El contenido que escribes o envías para chat con tutor, práctica de escritura, escritura a mano o reconocimiento de imagen puede enviarse a proveedores de IA de terceros configurados (por ejemplo DeepSeek, OpenAI, Anthropic Claude o Google Gemini) para generar respuestas, correcciones o resultados de reconocimiento. No almacenamos prompts sin procesar ni audio de voz en nuestros servidores; el tratamiento por parte de los proveedores se rige por sus condiciones de tratamiento de datos y la configuración actual del proyecto.\n(e) Datos del dispositivo y registro — registros de fallos, tipo de SO, versión de la app. Se usan solo para seguridad y calidad.\n(f) Token de notificación — si activas el recordatorio diario, guardamos un token push de Expo asociado a tu dispositivo.\n\nNo recopilamos a sabiendas datos de menores de 14 años (Corea) ni de menores de 16 años (EEE).",
    id: "Kami hanya mengumpulkan informasi minimum yang diperlukan untuk menjalankan Layanan.\n\n(a) Informasi akun — alamat emailmu (melalui Google OAuth atau tautan ajaib), ID pengguna yang diberikan Supabase saat pendaftaran, dan, jika kamu memakai login Google, informasi profil yang diberikan Google (nama tampilan, URL avatar).\n(b) Progres belajar — XP, level, streak harian, jumlah kata yang dipelajari, tanggal sesi terakhir, serta pengaturan bahasa asli dan bahasa target. Data ini disimpan di tabel linguaai_user_progress di Supabase.\n(c) Rekaman suara — Saat kamu merekam diri untuk latihan pengucapan, audio dikirim ke Microsoft Azure Speech untuk dinilai. Kami tidak menyimpan rekaman di server kami, dan Azure menerapkan kebijakan \"tanpa penyimpanan\" serta menghapus audio setelah penilaian.\n(d) Teks percakapan / tulisan / gambar — Konten yang kamu ketik atau kirim untuk chat tutor, latihan menulis, tulisan tangan, atau pengenalan gambar dapat dikirim ke penyedia AI pihak ketiga yang dikonfigurasi (misalnya DeepSeek, OpenAI, Anthropic Claude, atau Google Gemini) untuk menghasilkan balasan, koreksi, atau hasil pengenalan. Kami tidak menyimpan prompt mentah atau audio suara di server kami; penanganan oleh penyedia mengikuti ketentuan pemrosesan data masing-masing penyedia dan pengaturan proyek saat ini.\n(e) Data perangkat dan log — log kerusakan (crash), jenis sistem operasi, versi aplikasi. Hanya digunakan untuk keamanan dan kualitas produk.\n(f) Token notifikasi — jika kamu mengaktifkan pengingat harian, kami menyimpan token push Expo yang terkait dengan perangkatmu.\n\nKami tidak dengan sengaja mengumpulkan data dari anak di bawah 14 tahun (Korea) atau 16 tahun (EEA).",
  },

  purposes_h:   { ko: "4. 처리 목적",                           en: "4. Purposes of Processing",            es: "4. Finalidades del Tratamiento",       id: "4. Tujuan Pemrosesan" },
  purposes_b: {
    ...
    id: "Kami menggunakan informasi tersebut hanya untuk:\n  - mengidentifikasimu dan menjaga sesimu tetap masuk;\n  - menyinkronkan progres belajarmu (XP / level / streak) antar perangkat;\n  - menilai pengucapan dan menghasilkan respons tutor;\n  - mengirim notifikasi push yang kamu aktifkan;\n  - mencegah penyalahgunaan, menangani insiden keamanan, dan memenuhi kewajiban hukum;\n  - analitik agregat dan anonim untuk meningkatkan Layanan.\n\nKami tidak menggunakan informasi pribadimu untuk tujuan lain apa pun, dan tidak menggunakannya untuk pemasaran atau iklan tanpa persetujuanmu yang terpisah dan tegas.",
  },

  basis_h:      { ko: "5. 처리의 법적 근거 (GDPR 제6조)",       en: "5. Legal Basis (GDPR Art. 6)",         es: "5. Base Jurídica (Art. 6 RGPD)",       id: "5. Dasar Hukum (Pasal 6 GDPR)" },
  basis_b: {
    ...
    id: "Untuk pengguna di UE/EEA, dasar hukum pemrosesan adalah:\n  - data akun dan progres belajar: pelaksanaan kontrak (Pasal 6(1)(b) GDPR) — diperlukan untuk menyediakan Layanan yang kamu daftari;\n  - penilaian pengucapan dan respons AI: pelaksanaan kontrak (Pasal 6(1)(b));\n  - notifikasi push: persetujuanmu (Pasal 6(1)(a)) — dapat ditarik kapan saja di pengaturan;\n  - log keamanan dan pencegahan penyalahgunaan: kepentingan sah (Pasal 6(1)(f));\n  - pemrosesan apa pun yang diwajibkan hukum: kewajiban hukum (Pasal 6(1)(c)).",
  },

  subproc_h:    { ko: "6. 처리 위탁 및 제3자 제공(수탁자)",     en: "6. Subprocessors",                     es: "6. Subprocesadores",                  id: "6. Subprosesor" },
  subproc_b: {
    ko: "서비스 제공을 위해 다음 수탁자에게 일부 처리를 위탁합니다. 각 수탁자는 본인 정보를 자기 목적으로 사용할 수 없으며, 서비스 종료 또는 위탁계약 해지 시 보관 중인 데이터는 즉시 파기됩니다.\n\n  • Supabase Inc. (미국) — 인증 및 PostgreSQL 데이터베이스 호스팅. 회원 이메일, 사용자 ID, 학습 진도가 저장됩니다.\n  • DeepSeek, OpenAI, Anthropic, Google Gemini 등 설정된 AI 제공자 — AI 튜터 응답, 첨삭, 번역, 손글씨·이미지 인식. 사용자가 입력한 텍스트 또는 이미지 내용이 전송될 수 있습니다.\n  • Microsoft Azure Cognitive Services (미국·EU 리전) — 발음 채점. 녹음된 음성이 전송되며 채점 후 즉시 폐기됩니다.\n  • Google Cloud Text-to-Speech (미국) — TTS 음성 합성을 위해 학습 콘텐츠 텍스트가 전송됩니다.\n  • Vercel Inc. (미국) — 웹 호스팅 및 CDN.\n  • Railway Corp. (미국) — API 서버 호스팅.\n  • Expo / EAS (미국) — 푸시 알림 토큰 관리, 앱 빌드 배포.\n\n이상의 위탁은 「개인정보 보호법」 제26조에 따른 처리 위탁이며, EU/EEA 거주자의 데이터는 GDPR 제46조에 따라 표준계약조항(SCCs) 또는 충분성 결정을 기반으로 국외 이전됩니다.",
    en: "We use the following subprocessors. Each is contractually bound to use your data only on our instructions and to delete it when the engagement ends.\n\n  • Supabase Inc. (USA) — authentication and PostgreSQL hosting. Stores your email, user ID, and learning progress.\n  • Configured AI providers such as DeepSeek, OpenAI, Anthropic, and Google Gemini — AI tutor replies, corrections, translation, handwriting, and image recognition. Text or image content you submit may be sent to these providers.\n  • Microsoft Azure Cognitive Services (USA / EU regions) — pronunciation scoring. Voice audio is sent, scored, and immediately discarded.\n  • Google Cloud Text-to-Speech (USA) — converts lesson text to spoken audio.\n  • Vercel Inc. (USA) — web hosting and CDN.\n  • Railway Corp. (USA) — API server hosting.\n  • Expo / EAS (USA) — push-token management and app build distribution.\n\nThese arrangements are processing entrustments under PIPA Article 26. For EU/EEA users, transfers outside the EEA rely on Standard Contractual Clauses (GDPR Art. 46) or applicable adequacy decisions.",
    es: "Utilizamos los siguientes subprocesadores. Cada uno está obligado contractualmente a usar tus datos solo bajo nuestras instrucciones y a eliminarlos al terminar la relación.\n\n  • Supabase Inc. (EE. UU.) — autenticación y base de datos PostgreSQL. Guarda tu correo, ID de usuario y progreso.\n  • Proveedores de IA configurados como DeepSeek, OpenAI, Anthropic y Google Gemini — respuestas del tutor IA, correcciones, traducción, escritura a mano y reconocimiento de imagen. El texto o contenido de imagen que envías puede transmitirse a estos proveedores.\n  • Microsoft Azure Cognitive Services (EE. UU. / regiones de la UE) — evaluación de pronunciación. La voz se envía, se evalúa y se descarta de inmediato.\n  • Google Cloud Text-to-Speech (EE. UU.) — convierte texto de la lección a audio.\n  • Vercel Inc. (EE. UU.) — alojamiento web y CDN.\n  • Railway Corp. (EE. UU.) — alojamiento del servidor de la API.\n  • Expo / EAS (EE. UU.) — gestión de tokens push y distribución de builds.\n\nEstos acuerdos constituyen encargos del tratamiento conforme al art. 26 de PIPA. Para usuarios del EEE, las transferencias fuera del EEE se basan en Cláusulas Contractuales Tipo (art. 46 del RGPD) o decisiones de adecuación.",
    id: "Kami menggunakan subprosesor berikut. Masing-masing terikat kontrak untuk memakai datamu hanya atas instruksi kami dan menghapusnya saat kerja sama berakhir.\n\n  • Supabase Inc. (AS) — autentikasi dan hosting PostgreSQL. Menyimpan emailmu, ID pengguna, dan progres belajar.\n  • Penyedia AI yang dikonfigurasi seperti DeepSeek, OpenAI, Anthropic, dan Google Gemini — balasan tutor AI, koreksi, terjemahan, tulisan tangan, dan pengenalan gambar. Teks atau konten gambar yang kamu kirim dapat dikirim ke penyedia ini.\n  • Microsoft Azure Cognitive Services (AS / wilayah UE) — penilaian pengucapan. Audio suara dikirim, dinilai, dan langsung dihapus.\n  • Google Cloud Text-to-Speech (AS) — mengubah teks pelajaran menjadi audio.\n  • Vercel Inc. (AS) — hosting web dan CDN.\n  • Railway Corp. (AS) — hosting server API.\n  • Expo / EAS (AS) — pengelolaan token push dan distribusi build aplikasi.\n\nPengaturan ini merupakan penyerahan pemrosesan berdasarkan Pasal 26 PIPA. Untuk pengguna UE/EEA, transfer ke luar EEA mengandalkan Klausul Kontraktual Standar (Pasal 46 GDPR) atau keputusan kecukupan yang berlaku.",
  },

  retention_h:  { ko: "7. 보유 및 이용 기간",                   en: "7. Retention Period",                  es: "7. Plazo de Conservación",            id: "7. Jangka Waktu Penyimpanan" },
  retention_b: {
    ko: "  - 계정 정보 및 학습 진도: 회원 탈퇴 시까지 보유합니다. 탈퇴 즉시 영구 삭제하며, 백업본은 최대 30일 이내 폐기됩니다.\n  - 음성 녹음: 회사 서버에 저장하지 않으며, Azure 측에서도 채점 즉시 폐기됩니다.\n  - 대화/작문/이미지 원문: 회사 서버에 저장하지 않으며, 각 AI 제공자의 데이터 처리 조건 및 현재 프로젝트 설정에 따라 처리됩니다.\n  - 크래시 로그: 최대 90일.\n  - 보안 사고 또는 법령상 보존 의무가 있는 경우 해당 기간 동안 별도 분리·저장됩니다(예: 통신비밀보호법, 전자상거래법 등).",
    en: "  - Account info and learning progress: kept while your account is active; permanently deleted on account closure, with backups purged within 30 days.\n  - Voice recordings: never stored on our servers; Azure discards them as soon as scoring is complete.\n  - Conversation / writing / image source content: not stored on our servers; provider retention is governed by each AI provider's data-processing terms and the current project settings.\n  - Crash logs: up to 90 days.\n  - Where Korean or EU law requires longer retention (e.g. for a security investigation, the Korean Act on Electronic Commerce, or the Communications Privacy Act), the affected data is segregated and retained for the required period.",
    es: "  - Información de la cuenta y progreso: se conserva mientras la cuenta esté activa; se elimina permanentemente al cerrar la cuenta y las copias de seguridad se purgan en 30 días.\n  - Grabaciones de voz: nunca se almacenan en nuestros servidores; Azure las descarta tras la evaluación.\n  - Contenido fuente de conversación / escritura / imagen: no se almacena en nuestros servidores; la retención del proveedor se rige por las condiciones de tratamiento de datos de cada proveedor de IA y la configuración actual del proyecto.\n  - Registros de fallos: hasta 90 días.\n  - Cuando la ley coreana o de la UE exija una conservación más larga (por ej., una investigación de seguridad, la Ley de Comercio Electrónico o la Ley de Privacidad de las Comunicaciones), los datos afectados se segregan y conservan por el plazo requerido.",
    id: "  - Informasi akun dan progres belajar: disimpan selama akunmu aktif; dihapus permanen saat akun ditutup, dengan cadangan dihapus dalam 30 hari.\n  - Rekaman suara: tidak pernah disimpan di server kami; Azure menghapusnya segera setelah penilaian selesai.\n  - Konten sumber percakapan / tulisan / gambar: tidak disimpan di server kami; retensi penyedia mengikuti ketentuan pemrosesan data masing-masing penyedia AI dan pengaturan proyek saat ini.\n  - Log kerusakan (crash): hingga 90 hari.\n  - Jika hukum Korea atau UE mewajibkan penyimpanan lebih lama (mis. untuk investigasi keamanan, UU Perdagangan Elektronik Korea, atau UU Privasi Komunikasi), data terkait dipisahkan dan disimpan selama jangka waktu yang diwajibkan.",
  },

  rights_h:     { ko: "8. 정보주체의 권리",                     en: "8. Your Rights",                       es: "8. Tus Derechos",                     id: "8. Hak-Hakmu" },
  rights_b: {
    ...
    id: "Berdasarkan Pasal 35 dst. PIPA dan Pasal 15–22 GDPR, kamu berhak untuk:\n  - Akses — menerima salinan data pribadi yang kami simpan tentangmu;\n  - Perbaikan dan penghapusan — mengoreksi data yang tidak akurat, atau menghapus akunmu secara permanen (\"hak untuk dilupakan\") lewat tombol Hapus di dalam aplikasi;\n  - Portabilitas (GDPR) — mengunduh data yang kamu berikan dalam format terstruktur dan terbaca mesin (\"Unduh data saya\" di dalam aplikasi);\n  - Membatasi pemrosesan dan menarik persetujuan — untuk pemrosesan berbasis persetujuan seperti pemasaran atau notifikasi, kapan saja;\n  - Menolak pengambilan keputusan otomatis (Pasal 22 GDPR) — penilaian kami hanyalah alat bantu, bukan keputusan otomatis yang mengikat; kami tidak membuat keputusan yang menimbulkan akibat hukum semata-mata dari otomatisasi.\n\nUntuk menggunakan hak-hak ini, buka Pengaturan → \"Data Saya / Privasi & Ketentuan\" di dalam aplikasi, atau tulis ke privacy@linguaai.example. Kami merespons dalam 10 hari berdasarkan hukum Korea dan dalam satu bulan berdasarkan GDPR (dapat diperpanjang hingga dua bulan lagi untuk permintaan yang kompleks).",
  },

  security_h:   { ko: "9. 안전성 확보 조치",                    en: "9. Security Measures",                 es: "9. Medidas de Seguridad",             id: "9. Langkah Keamanan" },
  security_b: {
    ...
    id: "Kami mengikuti \"Standar Langkah Keamanan\" PIPC Korea dan menerapkan setidaknya hal berikut:\n  - TLS 1.2+ untuk semua lalu lintas dalam transit;\n  - kebijakan Row-Level Security pada Postgres sehingga setiap pengguna hanya bisa membaca barisnya sendiri;\n  - prinsip hak akses seminimal mungkin dan peninjauan akses tiap kuartal;\n  - kewajiban kerahasiaan bagi staf;\n  - pencadangan rutin dengan verifikasi integritas;\n  - proses pemberitahuan pelanggaran yang memenuhi hukum Korea (dalam 5 hari) dan GDPR (dalam 72 jam).",
  },

  cookies_h:    { ko: "10. 쿠키 및 로컬 저장소",                en: "10. Cookies and Local Storage",        es: "10. Cookies y Almacenamiento Local",  id: "10. Cookie dan Penyimpanan Lokal" },
  cookies_b: {
    ...
    id: "Di web kami memakai cookie yang benar-benar diperlukan dan localStorage untuk menjaga sesimu tetap masuk dan menyinkronkan progres. Kami tidak memakai cookie iklan atau analitik pihak ketiga. Di aplikasi native, data yang setara disimpan di AsyncStorage pada perangkatmu.",
  },

  intltransfer_h: { ko: "11. 국외 이전",                          en: "11. International Transfers",        es: "11. Transferencias Internacionales",  id: "11. Transfer Internasional" },
  intltransfer_b: {
    ...
    id: "Beberapa subprosesor yang disebut di atas beroperasi terutama di Amerika Serikat, yang berarti datamu ditransfer ke luar Korea / luar EEA saat kamu menggunakan Layanan. Kategori, negara penerima, waktu transfer (saat kamu menggunakan Layanan), metode (HTTPS), identitas penerima, dan langkah pengamanan diungkapkan dalam Kebijakan ini. Transfer mematuhi Pasal 28-8 PIPA dan, untuk keperluan GDPR, mengandalkan Klausul Kontraktual Standar atau keputusan kecukupan.",
  },

  children_h:   { ko: "12. 만 14세 미만 아동",                   en: "12. Children Under 14",                es: "12. Menores de 14 Años",              id: "12. Anak di Bawah 14 Tahun" },
  children_b: {
    ...
    id: "Layanan ini tidak ditujukan untuk anak di bawah 14 tahun. Jika kami mengetahui telah mengumpulkan data dari anak di bawah 14 tahun, kami akan segera menghapus akun tersebut. Untuk pengguna di EEA, mereka yang di bawah 16 tahun harus memiliki persetujuan orang tua atau wali.",
  },

  complaint_h:  { ko: "13. 권리 침해 구제 방법",                en: "13. Complaints",                       es: "13. Reclamaciones",                   id: "13. Pengaduan" },
  complaint_b: {
    ...
    id: "Jika kamu merasa kami telah melanggar hak-hakmu, kamu dapat mengajukan pengaduan ke:\n  • Komite Mediasi Sengketa Informasi Pribadi Korea: +82-1833-6972, www.kopico.go.kr\n  • Pusat Pengaduan Privasi KISA: 118, privacy.kisa.or.kr\n  • Unit Kejahatan Siber Kejaksaan Korea: 1301, www.spo.go.kr\n  • Biro Siber Kepolisian Korea: 182, ecrm.cyber.go.kr\n\nPengguna di UE/EEA juga berhak mengajukan pengaduan ke otoritas pengawas setempat (mis. CNIL di Prancis, BfDI di Jerman). Pengguna di Indonesia dapat menghubungi tim kami di privacy@linguaai.example.",
  },

  changes_h:    { ko: "14. 방침의 변경",                        en: "14. Changes to this Policy",          es: "14. Cambios en esta Política",        id: "14. Perubahan Kebijakan Ini" },
  changes_b: {
    ...
    id: "Jika kami mengubah Kebijakan ini secara material, kami akan memberitahumu di dalam aplikasi dan lewat email setidaknya 7 hari sebelum perubahan berlaku (30 hari untuk perubahan signifikan). Melanjutkan penggunaan setelah tanggal tersebut berarti menerima perubahan.",
  },

  contact_h:    { ko: "15. 문의",                                en: "15. Contact",                          es: "15. Contacto",                        id: "15. Kontak" },
  contact_b: {
    ...
    id: "Semua pertanyaan terkait privasi harap dikirim ke privacy@linguaai.example. (Alamat ini sementara dan akan diganti dengan alamat operasional kami yang sebenarnya sebelum peluncuran.)",
  },
};
