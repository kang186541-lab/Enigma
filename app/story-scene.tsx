  ...
  london: {
    ...
    sequence: [
      ...
      },
      {
        ...
        hints: {
          h1: {
            ko: "문장 1은 'The'로 시작해. 문장 2는 'Language'로 시작해",
            en: "Sentence 1 starts with 'The'. Sentence 2 starts with 'Language'",
            es: "La frase 1 empieza con 'The'. La frase 2 empieza con 'Language'",
            byLearning: {
              spanish: { ko: "문장 1은 'El'로 시작해. 문장 2도 'El'로 시작해", en: "Sentence 1 starts with 'El'. Sentence 2 also starts with 'El'", es: "La frase 1 empieza con 'El'. La frase 2 también empieza con 'El'" },
              korean:  { ko: "문장 1은 '그'로 시작해. 문장 2는 '언어는'으로 시작해", en: "Sentence 1 starts with '그'. Sentence 2 starts with '언어는'", es: "La frase 1 empieza con '그'. La frase 2 empieza con '언어는'" },
            },
          },
          h2: {
            ko: "문장 1의 핵심 동사는 'appeared'. 문장 2의 핵심 동사는 'is being used'",
            en: "Sentence 1 main verb: 'appeared'. Sentence 2 main verb: 'is being used'",
            es: "Verbo principal de la frase 1: 'appeared'. Verbo de la frase 2: 'is being used'",
            byLearning: {
              spanish: { ko: "문장 1의 핵심 동사는 'apareció'. 문장 2의 핵심 동사는 'está siendo usado'", en: "Sentence 1 main verb: 'apareció'. Sentence 2 main verb: 'está siendo usado'", es: "Verbo principal de la frase 1: 'apareció'. Verbo de la frase 2: 'está siendo usado'" },
              korean:  { ko: "문장 1의 핵심 동사는 '나타났다'. 문장 2의 핵심 동사는 '사용되고 있다'", en: "Sentence 1 main verb: '나타났다'. Sentence 2 main verb: '사용되고 있다'", es: "Verbo de la frase 1: '나타났다'. Verbo de la frase 2: '사용되고 있다'" },
            },
          },
          h3: {
            ko: "S1: The → symbol → appeared → at → every → crime → scene. S2: Language → is → being → used → as → a → weapon",
            en: "S1: The → symbol → appeared → at → every → crime → scene. S2: Language → is → being → used → as → a → weapon",
            es: "F1: The → symbol → appeared → at → every → crime → scene. F2: Language → is → being → used → as → a → weapon",
            byLearning: {
              spanish: { ko: "S1: El → símbolo → apareció → en → cada → escena → del crimen. S2: El → lenguaje → está → siendo → usado → como → arma", en: "S1: El → símbolo → apareció → en → cada → escena → del crimen. S2: El → lenguaje → está → siendo → usado → como → arma", es: "F1: El → símbolo → apareció → en → cada → escena → del crimen. F2: El → lenguaje → está → siendo → usado → como → arma" },
              korean:  { ko: "S1: 그 → 기호가 → 나타났다 → 모든 → 범죄 → 현장에서. S2: 언어는 → 사용되고 → 있다 → 무기로", en: "S1: 그 → 기호가 → 나타났다 → 모든 → 범죄 → 현장에서. S2: 언어는 → 사용되고 → 있다 → 무기로", es: "F1: 그 → 기호가 → 나타났다 → 모든 → 범죄 → 현장에서. F2: 언어는 → 사용되고 → 있다 → 무기로" },
            },
          },
        },
      },
  ...
  seoul: {
    ...
    sequence: [
      ...
      },
      {
        ...
        hints: {
          h1: {
            ko: "문장은 'language'로 시작해",
            en: "The sentence starts with 'language'",
            es: "La frase empieza con 'language'",
            byLearning: {
              spanish: { ko: "문장은 'el'로 시작해 (소문자 관사)", en: "The sentence starts with 'el' (lowercase article)", es: "La frase empieza con 'el' (artículo en minúscula)" },
              korean:  { ko: "문장은 '언어는'으로 시작해", en: "The sentence starts with '언어는'", es: "La frase empieza con '언어는'" },
            },
          },
          h2: {
            ko: "핵심 동사는 'is'야 — 주어와 보어를 연결하는 동사 (language IS the key)",
            en: "The main verb is 'is' — it links subject to meaning (language IS the key)",
            es: "El verbo principal es 'is' — conecta sujeto y complemento (language IS the key)",
            byLearning: {
              spanish: { ko: "핵심 동사는 'es'야 — 주어와 보어를 연결해 (el lenguaje ES la clave)", en: "The main verb is 'es' — it links subject to meaning (el lenguaje ES la clave)", es: "El verbo principal es 'es' — conecta sujeto y complemento (el lenguaje ES la clave)" },
              korean:  { ko: "핵심 동사는 '이다'야 — 주어와 보어를 연결해 (언어는 열쇠 '이다')", en: "The main verb is '이다' — it links subject to meaning (언어는 열쇠 '이다')", es: "El verbo principal es '이다' — conecta sujeto y complemento (언어는 열쇠 '이다')" },
            },
          },
          h3: {
            ko: "구조: language(주어) → is(동사) → the(관사) → key(보어) → to(전치사) → freedom(목적어)",
            en: "Structure: language(subject) → is(verb) → the(article) → key(complement) → to(prep) → freedom(object)",
            es: "Estructura: language(sujeto) → is(verbo) → the(artículo) → key(complemento) → to(prep) → freedom(objeto)",
            byLearning: {
              spanish: { ko: "구조: el(관사) → lenguaje(주어) → es(동사) → la(관사) → clave(보어) → de la libertad(수식어)", en: "Structure: el(article) → lenguaje(subject) → es(verb) → la(article) → clave(complement) → de la libertad(modifier)", es: "Estructura: el(artículo) → lenguaje(sujeto) → es(verbo) → la(artículo) → clave(complemento) → de la libertad(modificador)" },
              korean:  { ko: "구조: 언어는(주어) → 이다(동사) → 열쇠(보어) → 에(조사) → 자유(목적어)", en: "Structure: 언어는(subject) → 이다(verb) → 열쇠(complement) → 에(particle) → 자유(object)", es: "Estructura: 언어는(sujeto) → 이다(verbo) → 열쇠(complemento) → 에(partícula) → 자유(objeto)" },
            },
          },
        },
      },
